...
1. **CSV 内部去重**：CSV 文件内重复的行只保留第一条
2. **与数据库去重**：数据库中已存在的词条会被跳过，仅插入新增的

去重依据（可在 `import_sql.py` 顶部 `DEDUP_BY_BOTH` 切换）：
- `True`（默认）：按 **topic + plaintext** 组合判断（标题+正文都一样才算重复）
- `False`：仅按 **topic** 判断（标题相同就算重复）

输出示例：

```
📂 读取 CSV：Second Mind_个人语录.csv
   CSV 原始行数：1200
🔍 CSV 内部去重（topic+plaintext）：去掉 50 条，剩余 1150 条
🔗 连接数据库，查询已有词条...
   数据库已有：1062 条
🚫 与数据库重复（跳过）：1000 条
✨ 待新增：150 条
